...
sleep 3
tmux send-keys -t ccq:cli "exec node src/control-cli.ts --log-dir /tmp/ccc-q-logs --server http://127.0.0.1:8790 --credential q-cred" Enter
sleep 18
tmux send-keys -t ccq:cli "/rc" Enter
sleep 8
echo "=== driving the question from web ==="
node test/question-client.ts q-cred 8790
RC=$?
